...
class Connection extends Component
{
    ...
    public function open()
    {
        ...
        $this->_redis->pconnect($this->hostname, $this->port, $this->connectionTimeout ? $this->connectionTimeout : ini_get("default_redis_timeout"));
        if ($this->_redis) {
            ...
            \Yii::error("Failed to open redis DB connection :".$this->_redis, __CLASS__);
            $message = YII_DEBUG ? "Failed to open redis DB connection :".$this->_redis : 'Failed to open DB connection.';
    ...
    public function executeCommand($name, $params = [])
    {
        ...
        $arg2 = isset($params[2]) ? strtolower($params[2]) : null;
...
//        print_r($params);
        ...
        switch ($name) {
            ...
            case 'zrank':
            case 'zrevrank':
                return $this->_redis->{$name}($params[0], $params[1]);
            ...
            case 'srandmember':
            case 'smembers':
                return $this->_redis->{$name}($params[0]);
            ...
            case 'zadd':
            case 'zincrby':
                return $this->_redis->{$name}($params[0], $params[1], $arg2);

            case 'incr':
                if (isset($params[1])) {
                    return $this->_redis->incrBy($params[0], $params[1]);
                }else{
                    //default step=1
                    return $this->_redis->incr($params[0]);
                }

            case 'decr':
                if (isset($params[1])) {
                    return $this->_redis->decrBy($params[0], $params[1]);
                }else {
                    return $this->_redis->decr($params[0]);
                }
            ...
            case 'hmset':
                if (isset($params[1]) && is_array($params[1])) {
                    return $this->_redis->hmset($params[0], $params[1]);
                }else{
                    $tempArray = $params;
                    array_shift($tempArray);
                    if (!empty($tempArray)) {
                        $data = $this->buildMSet($tempArray);
                        if ($data) {
                            return $this->_redis->hmset($params[0], $data);
                        }
                    }
                    return null;
                }
            ...
            case 'hmget':
                if (isset($params[1]) && is_array($params[1])) {
                    return $this->_redis->hmGet($params[0], $params[1]);
                }else {
                    $tempArray = $params;
                    array_shift($tempArray);
                    $data = $this->_redis->hmGet($params[0], $tempArray);
                    return array_values($data);
                }

            case 'hgetall':
                $data = $this->_redis->hGetAll($params[0]);
                return $this->parseResponse($data);

            case 'setex':
            case 'psetex':
                //key expire value
                return $this->_redis->{$name}($params[0], $arg2, $params[1]);
            ...
            case 'hdel':
                $tempArray = $params;
                array_shift($tempArray);
                return $this->_redis->{$name}($params[0], ...$tempArray);
            ...
            case 'linsert':
                if (isset($params[3])) {
                    return $this->_redis->{$name}($params[0], $params[1], $params[2], $params[3]);
                }else{
                    return $this->_redis->{$name}($params[0], $params[1], $params[2]);
                }
            ...
            case 'saddarray':
                ...
                if (count($tempArray) > 1) {
                    return $this->_redis->sAddArray($params[0], $tempArray);
                }else {
                    return $this->_redis->sAdd($params[0], $params[1]);
                }
     ...
     * @return array|null
     */
    private function buildMSet($params){
